...
android {
    ...
    // ndkVersion = providers.gradleProperty("mhNdkVersion").orNull ?: "29.0.14206865"

    signingConfigs {
        val rootDebugKeystore = rootProject.file("debug.keystore")
        if (rootDebugKeystore.isFile) {
            getByName("debug") {
    ...
    defaultConfig {
        ...
        manifestPlaceholders["appName"] = "Mobile Harness"
    ...
    buildTypes {
        debug {
            val suffix = providers.gradleProperty("appIdSuffix").orNull ?: ".dev"
            if (suffix.isNotBlank()) {
                applicationIdSuffix = suffix
                versionNameSuffix = "-dev"
                manifestPlaceholders["appName"] = "Mobile Harness (Dev)"
            } else {
                manifestPlaceholders["appName"] = "Mobile Harness"
            }
