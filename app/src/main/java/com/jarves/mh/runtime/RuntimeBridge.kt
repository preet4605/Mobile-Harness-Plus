    ...
    fun build(profile: ProviderProfile, authToken: String? = null, localGatewayUrl: String? = null): RuntimeLaunchConfig {
        ...
        when (profile.kind.protocol) {
            ...
            com.jarves.mh.model.ProviderProtocol.ANTHROPIC_GATEWAY -> {
                if (profile.kind == com.jarves.mh.model.ProviderKind.ANTIGRAVITY_SERVER) {
                    environment["ANTHROPIC_BASE_URL"] = (localGatewayUrl ?: profile.baseUrl).trimEnd('/')
                    environment["ANTHROPIC_MODEL"] = profile.model.ifBlank { "gemini-3.8-pro" }
                } else {
                    environment["ANTHROPIC_BASE_URL"] = profile.baseUrl.trimEnd('/')
                    environment["ANTHROPIC_MODEL"] = profile.model
                }
        ...
        if (profile.kind.protocol != com.jarves.mh.model.ProviderProtocol.CLAUDE_LOGIN) {
            ...
            val effectiveAuthToken = if (profile.kind == com.jarves.mh.model.ProviderKind.ANTIGRAVITY_SERVER) {
                authToken?.ifBlank { "antigravity-local-token" } ?: "antigravity-local-token"
            } else {
                authToken
            }
            if (!effectiveAuthToken.isNullOrBlank()) {
                environment["ANTHROPIC_AUTH_TOKEN"] = effectiveAuthToken
                if (profile.kind == com.jarves.mh.model.ProviderKind.LLM_ROUTER) {
                    environment["ANTHROPIC_API_KEY"] = ""
                    environment["OPENROUTER_API_KEY"] = effectiveAuthToken
                } else {
                    environment["ANTHROPIC_API_KEY"] = effectiveAuthToken
                }
            }
        }
